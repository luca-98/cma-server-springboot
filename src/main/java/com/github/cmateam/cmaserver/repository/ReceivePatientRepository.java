...
public interface ReceivePatientRepository extends JpaRepository<ReceivePatientEntity, UUID> {
	...
	@Query("SELECT count(r.id) FROM ReceivePatientEntity r")
	...
	@Query("FROM ReceivePatientEntity r JOIN r.patientByPatientId p WHERE p.patientCode=?1 AND r.status NOT IN(0,5) AND date_trunc('day', r.createdAt)=date_trunc('day', now())")
	ReceivePatientEntity findReceiveByPatientCode(String patientCode);

	@Query("FROM ReceivePatientEntity r JOIN r.patientByPatientId p WHERE p.phone=?1 AND r.status NOT IN(0,5) AND date_trunc('day', r.createdAt)=date_trunc('day', now())")
